...
public class GPUtil {
    ...
    public static byte[] ecb3des(byte[] key, byte[] text, int offset, int length) throws CardException {
        if (length == -1)
            length = text.length - offset;
        if((length%8)!=0)
        ...
        try {
            ICipher cipher = ICipher.Factory.getImplementation(
                    ICipher.DESEDE_ECB_NOPADDING, getKey(key, 24));
            return cipher.encrypt(text, offset, length);
        } catch (Exception e) {
            throw new CardException("ENC computation failed.");
        }
    }
